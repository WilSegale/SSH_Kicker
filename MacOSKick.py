            ...
            with open("KICKED_USERS.txt", "a") as kicked_users_file:
                ...
                if platform.system() == "Linux":
                    ...
                    if ssh in yes:
                        ...
                        time.sleep(2)
                        subprocess.run(['sudo', 'service', 'ssh', 'status'], check=True)
                    else:
                        print("Ok, SSH will still run.")
                else:
                    print("SSH shutdown is only supported on Linux.")
        except subprocess.CalledProcessError:
...
def main():
    ...
    if nslookupOrKick in nslookupCommand:
        ...
        ip_address = input("Enter the IP address of the user you want to kick off: ").strip()
